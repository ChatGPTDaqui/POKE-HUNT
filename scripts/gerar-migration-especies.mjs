...
-- deixa idempotente.
alter table ${schema}.species drop constraint if exists species_dex_number_check;
alter table ${schema}.species add constraint species_dex_number_check
  check (dex_number >= 1 and dex_number <= 386);

alter table ${schema}.species drop constraint if exists species_growth_curve_check;
alter table ${schema}.species add constraint species_growth_curve_check
  check (growth_curve in ('MEDIUM_FAST', 'MEDIUM_SLOW', 'FAST', 'SLOW', 'ERRATIC', 'FLUCTUATING'));

-- ---------------------------------------------------------------------------
-- 3. Especies (${novas.length})
-- ---------------------------------------------------------------------------
