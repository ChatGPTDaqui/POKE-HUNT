// A* route search over a hunt's collision grid (js/data/collisionGrids.generated.js).
...
//   precise arrival.
export function findPath(mapDef, startX, startY, goalX, goalY) {
  const grid = mapDef.collisionGrid;
  if (!grid) return null;
  const circle = { cx: mapDef.bounds.width / 2, cy: mapDef.bounds.height / 2, radius: mapWalkRadius(mapDef) };

  const toCol = (x) => Math.floor(x / COLLISION_GRID_CELL_SIZE);
  const toRow = (y) => Math.floor(y / COLLISION_GRID_CELL_SIZE);
  ...
  const open = new Map([[startKey, heuristic(startCol, startRow, goalCol, goalRow)]]);
  ...
  while (open.size > 0) {
    ...
    if (currentKey === goalKey) return reconstructPath(cameFrom, goalKey, startKey).map((wp, i, arr) => (i === arr.length - 1 ? { x: goalX, y: goalY } : wp));
