...
export function rarityOf(poke) {
  return (poke && RARITIES[poke.rarity]) || RARITIES.comum;
}

// Ordinal rank (common -> mythic), for sort UIs — distinct from `weight`,
// which runs the opposite direction (more common = higher weight).
export const RARITY_ORDER = ['comum', 'incomum', 'raro', 'ultra', 'legendary', 'mythic'];

export function rarityRank(key) {
