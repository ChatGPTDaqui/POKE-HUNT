...
--
-- Gemeo public: `20260823080000_aviso_de_negocio_a_contraparte_public.sql`.
--
...
-- 1. Negocio fechado
-- ===========================================================================
create or replace function dev.avisar_negocio_a_contraparte()
returns trigger
...
begin
  ...
  if new.kind = 'item' then
    select coalesce(name, new.item_id) into v_o_que from dev.items where id = new.item_id;
    v_o_que := coalesce(v_o_que, new.item_id) || ' x' || new.quantity;
  else
    select coalesce(name, new.species_id) into v_o_que from dev.species where id = new.species_id;
...
drop trigger if exists market_trades_avisa_contraparte on dev.market_trades;
create trigger market_trades_avisa_contraparte
  after insert on dev.market_trades
  for each row execute function dev.avisar_negocio_a_contraparte();
...
create or replace function dev.avisar_oferta_recusada()
returns trigger
language plpgsql security definer set search_path = dev, public
as $$
declare
  ...
  v_nome text;
begin
  select * into v_anuncio from dev.market_listings where id = new.listing_id;
  ...
  select coalesce(name, v_anuncio.species_id) into v_nome from dev.species where id = v_anuncio.species_id;
...
drop trigger if exists market_offers_avisa_recusa on dev.market_offers;
...
create trigger market_offers_avisa_recusa
  after update of status on dev.market_offers
  for each row
  when (new.status = 'recusada' and old.status = 'pendente')
  execute function dev.avisar_oferta_recusada();
