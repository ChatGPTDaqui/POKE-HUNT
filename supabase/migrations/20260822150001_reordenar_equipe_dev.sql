...
--
-- Par obrigatorio deste arquivo: 20260822150000_reordenar_equipe_public.sql.
...
create or replace function dev.reordenar_equipe(p_ordem uuid[])
returns jsonb
language plpgsql
security definer
...
revoke all on function dev.reordenar_equipe(uuid[]) from public;
revoke execute on function dev.reordenar_equipe(uuid[]) from anon;
grant execute on function dev.reordenar_equipe(uuid[]) to authenticated;
