-- PH-120 (espelho do schema dev), fatia 1: a SESSAO de troca direta entre dois jogadores.
--
...
create table if not exists dev.troca_sessao (
...
comment on table dev.troca_sessao is
...
create index if not exists troca_sessao_vivas_por_prazo
  on dev.troca_sessao (expira_em)
  where estado in ('convidada', 'aberta');

alter table dev.troca_sessao enable row level security;
...
drop policy if exists "troca leitura dos participantes" on dev.troca_sessao;
create policy "troca leitura dos participantes" on dev.troca_sessao
  for select to authenticated
  using (anfitriao_id = auth.uid() or convidado_id = auth.uid());

grant select on dev.troca_sessao to authenticated;
grant select, insert, update on dev.troca_sessao to service_role;

-- ---------------------------------------------------------------------------
-- 2. Abrir a mesa
-- ---------------------------------------------------------------------------
create or replace function dev.abrir_troca(p_convidado_id uuid)
...
begin
  ...
  perform 1 from dev.players where user_id = p_convidado_id;
  if not found then
  ...
  if dev.bloqueio_entre(v_eu, p_convidado_id) then
  ...
  perform 1 from dev.troca_sessao
   where estado in ('convidada', 'aberta')
  ...
  insert into dev.troca_sessao (anfitriao_id, convidado_id)
...
grant execute on function dev.abrir_troca(uuid) to authenticated;
grant execute on function dev.aceitar_troca(uuid) to authenticated;
grant execute on function dev.encerrar_troca(uuid, text) to authenticated;
...
-- dela.
grant execute on function dev.expirar_trocas() to service_role;
...
select cron.unschedule('trocas-expirar-dev') where exists (
  select 1 from cron.job where jobname = 'trocas-expirar-dev'
);
select cron.schedule('trocas-expirar-dev', '2-59/5 * * * *', $cron$select dev.expirar_trocas();$cron$);
