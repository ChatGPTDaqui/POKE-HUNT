-- PH-245 — espelho de 20260831190000_missao_cadeia_public.sql no schema `dev`.
-- O raciocinio completo esta na migration irma em `public`.
begin;

create table if not exists dev.missao_cadeia (
  ...
  -- SEM foreign key pra dev.species, de proposito: a cadeia e derivada do
...
-- Sem RLS de dono porque nao ha dono — e a mesma postura de `dev.species`.
alter table dev.missao_cadeia enable row level security;
drop policy if exists "leitura publica" on dev.missao_cadeia;
create policy "leitura publica" on dev.missao_cadeia for select to authenticated using (true);
grant select on dev.missao_cadeia to authenticated;
grant select, insert, update, delete on dev.missao_cadeia to service_role;
...
delete from dev.missao_cadeia;

insert into dev.missao_cadeia (tipo, species_id, posicao, alvo, recompensa, eh_ultima) values
...
create or replace function dev.reivindicar_missao(p_tipo text, p_species_id text)
returns jsonb
language plpgsql
security definer
set search_path = dev
...
begin
  ...
  select posicao, alvo, recompensa into v_posicao, v_alvo, v_recompensa
    from dev.missao_cadeia where tipo = v_tipo and species_id = p_species_id;
...
delete from dev.player_missoes_reivindicadas;
