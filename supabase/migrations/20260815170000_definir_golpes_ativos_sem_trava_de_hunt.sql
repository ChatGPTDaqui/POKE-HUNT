-- Pedido explicito do usuario: personalizar os 4 golpes ativos a qualquer
-- momento, inclusive dentro de uma hunt (o estado normal de quem joga um
-- idle) - so 'basic_attack' e o AOE do Nivel 50 escapavam da trava porque
-- passam por alternar_habilidade, sem esse bloqueio.
--
-- A trava "saia da hunt para trocar de golpe" (20260814120200, afinada em
-- 20260814120300) nao protege nada tecnico: cada janela de flush reconstroi
-- o mundo do ZERO a partir de pokemon_instances.active_abilities na hora do
-- flush (ver aplicarFlush/buildMapWorld) - trocar no meio da cacada so muda
-- o que a PROXIMA janela (<=30s depois) usa, sem corromper a que ja estava
-- rodando. Era decisao de design (nao trocar golpe "no meio da luta", espirito
-- dos jogos reais), nao trava de seguranca; revogada por pedido do usuario.
