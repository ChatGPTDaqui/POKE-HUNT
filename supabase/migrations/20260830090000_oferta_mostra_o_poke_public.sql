-- PH-314 (PH-120, fatia 4): a linha da oferta passa a DESCREVER o POKE.
--
...
alter table public.troca_oferta
  add column if not exists species_id text references public.species(id),
  add column if not exists level integer,
  add column if not exists is_shiny boolean,
  add column if not exists rarity public.rarity_tier,
  add column if not exists iv_percent integer;

comment on column public.troca_oferta.species_id is
...
begin
  ...
  insert into public.troca_oferta (
