-- PH-246 — espelho de 20260828150000_custo_especialidade_public.sql no schema dev.
...
begin
  ...
  v_stone_qtd_por_nivel := case p_tipo
    when 'NORMAL' then array[14, 32, 64, 118, 201]
    ...
    when 'ICE' then array[8, 19, 37, 69, 117]
    when 'FIGHTING' then array[4, 9, 17, 33, 55]
    when 'POISON' then array[8, 19, 39, 71, 121]
    when 'GROUND' then array[10, 24, 48, 89, 150]
    when 'FLYING' then array[8, 19, 37, 69, 117]
    when 'PSYCHIC' then array[16, 37, 75, 138, 234]
    when 'BUG' then array[2, 6, 12, 22, 37]
    when 'ROCK' then array[4, 9, 17, 32, 54]
    when 'GHOST' then array[3, 7, 14, 25, 43]
    when 'DRAGON' then array[4, 9, 19, 35, 58]
    when 'DARK' then array[4, 9, 19, 35, 58]
    when 'STEEL' then array[2, 3, 7, 13, 22]
    when 'FAIRY' then array[2, 6, 11, 21, 35]
  end;
