...
--
-- Par obrigatorio deste arquivo: 20260822140001_correio_amizade_e_dm_dev.sql.
...
drop policy if exists "bloqueio leitura propria" on public.blocks;
create policy "bloqueio leitura propria" on public.blocks
...
create index if not exists friend_messages_recebidas_idx
  on public.friend_messages (para_id, de_id, created_at desc);
create index if not exists friend_messages_enviadas_idx
  on public.friend_messages (de_id, para_id, created_at desc);
...
create index if not exists friend_messages_nao_lidas_idx
  on public.friend_messages (para_id, de_id)
  where read_at is null;

alter table public.friend_messages enable row level security;
grant select on public.friend_messages to authenticated;
grant select, insert, update, delete on public.friend_messages to service_role;

drop policy if exists "dm leitura das minhas conversas" on public.friend_messages;
create policy "dm leitura das minhas conversas" on public.friend_messages
...
begin
  ...
  select max(created_at) into v_ultima
    from public.friend_messages where de_id = new.de_id;
...
create trigger dm_rate_limit_trigger
  ...
  for each row execute function public.dm_rate_limit();
...
create index if not exists mail_messages_enviados_idx
  on public.mail_messages (de_id, created_at desc);
...
drop policy if exists "correio leitura de enviados" on public.mail_messages;
create policy "correio leitura de enviados" on public.mail_messages
...
create or replace function public.bloqueio_entre(p_a uuid, p_b uuid)
returns boolean
language sql
stable
security definer
...
create or replace function public.esta_online(p_user_id uuid)
returns boolean
language sql
stable
security definer
...
-- evitam contar.
revoke all on function public.bloqueio_entre(uuid, uuid) from public;
revoke execute on function public.bloqueio_entre(uuid, uuid) from anon, authenticated;
revoke all on function public.esta_online(uuid) from public;
revoke execute on function public.esta_online(uuid) from anon, authenticated;
revoke all on function public.dm_rate_limit() from public;
revoke execute on function public.dm_rate_limit() from anon, authenticated;
...
create or replace function public.enviar_correio(
  p_para_nick text,
  p_assunto text,
  p_corpo text,
...
revoke all on function public.enviar_correio(text, text, text, jsonb) from public;
revoke execute on function public.enviar_correio(text, text, text, jsonb) from anon;
grant execute on function public.enviar_correio(text, text, text, jsonb) to authenticated;
...
-- ser resposta.
create or replace function public.responder_correio(p_mensagem_id uuid, p_corpo text)
returns jsonb
language plpgsql
security definer
set search_path = public
...
declare
  v_user_id uuid := auth.uid();
  v_original public.mail_messages;
  v_destino record;
  v_assunto text;
begin
  if v_user_id is null then
  ...
  select * into v_original from public.mail_messages
  ...
  select trainer_name into v_destino from public.players where user_id = v_original.de_id;
  ...
  return public.enviar_correio(v_destino.trainer_name, v_assunto, p_corpo, '[]'::jsonb);
end;
$$;

revoke all on function public.responder_correio(uuid, text) from public;
revoke execute on function public.responder_correio(uuid, text) from anon;
grant execute on function public.responder_correio(uuid, text) to authenticated;
...
create or replace function public.excluir_correio(p_mensagem_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select * into v_msg from public.mail_messages
...
revoke all on function public.excluir_correio(uuid) from public;
revoke execute on function public.excluir_correio(uuid) from anon;
grant execute on function public.excluir_correio(uuid) to authenticated;
...
begin
  ...
  select trainer_name into v_nome from public.players where user_id = p_amigo_id;
...
revoke execute on function public.remover_amizade(uuid) from anon;
...
create or replace function public.bloquear_jogador(p_alvo_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select trainer_name into v_nome from public.players where user_id = p_alvo_id;
  if v_nome is null then
    raise exception 'Treinador nao encontrado.' using errcode = 'P0001';
  end if;

  insert into public.blocks (user_id, bloqueado_id)
...
revoke all on function public.bloquear_jogador(uuid) from public;
revoke execute on function public.bloquear_jogador(uuid) from anon;
grant execute on function public.bloquear_jogador(uuid) to authenticated;


create or replace function public.desbloquear_jogador(p_alvo_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from public.blocks where user_id = v_user_id and bloqueado_id = p_alvo_id;
  get diagnostics v_apagadas = row_count;
  if v_apagadas = 0 then
...
revoke all on function public.desbloquear_jogador(uuid) from public;
revoke execute on function public.desbloquear_jogador(uuid) from anon;
grant execute on function public.desbloquear_jogador(uuid) to authenticated;
...
create or replace function public.pedir_amizade(p_nick text)
returns jsonb
language plpgsql
security definer
set search_path = public
...
revoke execute on function public.pedir_amizade(text) from anon;
...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      'naoLidas', (
        select count(*) from public.friend_messages fm
...
create or replace function public.enviar_dm(p_para_id uuid, p_corpo text)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select exists(
    select 1 from public.friendships where user_id = v_user_id and amigo_id = p_para_id
  ) into v_amigos;
  if not v_amigos then
    raise exception 'Voce so pode conversar com amigos.' using errcode = 'P0001';
  end if;

  if public.bloqueio_entre(v_user_id, p_para_id) then
    raise exception 'Nao e possivel enviar mensagem para este treinador.' using errcode = 'P0001';
  end if;

  insert into public.friend_messages (de_id, para_id, corpo)
...
revoke all on function public.enviar_dm(uuid, text) from public;
revoke execute on function public.enviar_dm(uuid, text) from anon;
grant execute on function public.enviar_dm(uuid, text) to authenticated;


create or replace function public.marcar_dm_lidas(p_amigo_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
revoke all on function public.marcar_dm_lidas(uuid) from public;
revoke execute on function public.marcar_dm_lidas(uuid) from anon;
grant execute on function public.marcar_dm_lidas(uuid) to authenticated;
