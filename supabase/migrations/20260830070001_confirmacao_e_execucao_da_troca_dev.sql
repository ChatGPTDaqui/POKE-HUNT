-- PH-312 (espelho do schema dev): a confirmacao dupla e a execucao atomica.
--
...
comment on column dev.troca_sessao.versao_confirmada_anfitriao is
  'PH-312: em qual versao da oferta o anfitriao confirmou. Vale so enquanto for igual a versao atual.';
comment on column dev.troca_sessao.versao_confirmada_convidado is
...
create table if not exists dev.troca_log (
...
comment on table dev.troca_log is
...
create index if not exists troca_log_por_jogador
  on dev.troca_log (anfitriao_id, executada_em desc);
create index if not exists troca_log_por_convidado
  on dev.troca_log (convidado_id, executada_em desc);

alter table dev.troca_log enable row level security;

drop policy if exists "log de troca dos participantes" on dev.troca_log;
create policy "log de troca dos participantes" on dev.troca_log
  for select to authenticated
  using (anfitriao_id = auth.uid() or convidado_id = auth.uid());

grant select on dev.troca_log to authenticated;
grant select, insert on dev.troca_log to service_role;
...
create or replace function dev._executar_troca(p_sessao_id uuid)
...
declare
  ...
  v_destino uuid;
begin
  select * into v_s from dev.troca_sessao where id = p_sessao_id for update;
  if not found then
  ...
  for v_linha in
    select * from dev.troca_oferta where sessao_id = p_sessao_id and tipo = 'poke'
  loop
  ...
  insert into dev.troca_log (sessao_id, anfitriao_id, convidado_id, versao, oferta)
...
-- ---------------------------------------------------------------------------
create or replace function dev.confirmar_troca(p_sessao_id uuid, p_versao integer)
returns dev.troca_sessao
language plpgsql
security definer
set search_path to 'dev'
...
begin
  ...
  perform 1 from dev.troca_oferta where sessao_id = p_sessao_id;
  if not found then
    raise exception 'A mesa esta vazia.';
  end if;
  ...
  if v_sessao.versao_confirmada_anfitriao = v_sessao.versao
    ...
    v_sessao := dev._executar_troca(p_sessao_id);
  end if;
...
create or replace function dev.desconfirmar_troca(p_sessao_id uuid)
returns dev.troca_sessao
language plpgsql
security definer
set search_path to 'dev'
...
grant execute on function dev.confirmar_troca(uuid, integer) to authenticated;
grant execute on function dev.desconfirmar_troca(uuid) to authenticated;
...
grant execute on function dev._executar_troca(uuid) to service_role;
