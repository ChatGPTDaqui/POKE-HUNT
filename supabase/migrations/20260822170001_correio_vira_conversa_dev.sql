-- PAR DEV de 20260822170000_correio_vira_conversa_public.sql.
...
alter table dev.mail_messages alter column assunto drop not null;
...
create index if not exists mail_messages_fio_recebidas_idx
  on dev.mail_messages (para_id, de_id, created_at desc);
create index if not exists mail_messages_fio_enviadas_idx
  on dev.mail_messages (de_id, para_id, created_at desc);
...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      'naoLidas', (
        select count(*) from dev.mail_messages m
...
drop function if exists dev.dm_rate_limit();
drop function if exists dev.enviar_dm(uuid, text);
drop function if exists dev.marcar_dm_lidas(uuid);
drop table if exists dev.friend_messages;
...
create or replace function dev.enviar_mensagem(
  p_corpo text,
...
begin
  ...
  insert into dev.mail_messages (para_id, de_id, de_nome, tipo, assunto, corpo, estado, anexo_itens)
...
revoke all on function dev.enviar_mensagem(text, uuid, text, jsonb) from public;
revoke execute on function dev.enviar_mensagem(text, uuid, text, jsonb) from anon;
grant execute on function dev.enviar_mensagem(text, uuid, text, jsonb) to authenticated;
...
drop function if exists dev.enviar_correio(text, text, text, jsonb);
drop function if exists dev.responder_correio(uuid, text);
...
create or replace function dev.conversas()
returns jsonb
language plpgsql
security definer
set search_path = dev, public
...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      'online', dev.esta_online(u.contato),
      'bloqueado', dev.bloqueio_entre(v_user_id, u.contato)
    ) as linha
    from ultima u
    left join dev.players p on p.user_id = u.contato
...
revoke all on function dev.conversas() from public;
revoke execute on function dev.conversas() from anon;
grant execute on function dev.conversas() to authenticated;
...
create or replace function dev.marcar_conversa_lida(p_contato_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = dev, public
...
revoke all on function dev.marcar_conversa_lida(uuid) from public;
revoke execute on function dev.marcar_conversa_lida(uuid) from anon;
grant execute on function dev.marcar_conversa_lida(uuid) to authenticated;
...
create or replace function dev.excluir_conversa(p_contato_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = dev, public
...
revoke all on function dev.excluir_conversa(uuid) from public;
revoke execute on function dev.excluir_conversa(uuid) from anon;
grant execute on function dev.excluir_conversa(uuid) to authenticated;
