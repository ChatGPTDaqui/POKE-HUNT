...
--
-- Gemeo dev: `20260823050001_historico_de_preco_no_mercado_dev.sql`.
--
-- ===========================================================================
...
create index if not exists market_trades_por_especie_idx
  on public.market_trades (species_id, created_at desc)
...
create or replace view public.mercado_historico_itens with (security_invoker = true) as
select
  item_id,
  currency,
...
create or replace view public.mercado_historico_pokes with (security_invoker = true) as
select
  species_id,
  currency,
  (created_at at time zone 'utc')::date as dia,
...
-- passada so.
create or replace view public.mercado_resumo_historico_itens with (security_invoker = true) as
select
  item_id,
  currency,
...
create or replace view public.mercado_resumo_historico_pokes with (security_invoker = true) as
select
  species_id,
  currency,
  percentile_disc(0.5) within group (order by unit_price)
...
revoke all on public.mercado_historico_itens from public;
revoke all on public.mercado_historico_pokes from public;
revoke all on public.mercado_resumo_historico_itens from public;
revoke all on public.mercado_resumo_historico_pokes from public;
grant select on public.mercado_historico_itens to authenticated;
grant select on public.mercado_historico_pokes to authenticated;
grant select on public.mercado_resumo_historico_itens to authenticated;
grant select on public.mercado_resumo_historico_pokes to authenticated;
