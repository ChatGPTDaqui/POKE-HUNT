-- PH-330 — o Eevee do Campeao Lance passa a ser SORTEADO pelas regras do jogo,
-- e nao entregue com atributos fixos.
--
-- =========================================================================
-- O QUE ESTAVA CONGELADO, E O QUE ISSO CUSTAVA
-- =========================================================================
--
-- `_conceder_eevee_do_lance` (20260828230000) montava a receita a mao:
--
--     rarity 'comum', IV 23 nos seis, isShiny false, nature ausente
--
-- Tres consequencias, e a terceira nao estava nem escrita na issue original:
--
--   1. TODO jogador recebia o MESMO Eevee. O presente que devia ser "um POKE de
--      projeto" nao tinha nada de individual — nem raridade, nem IV, nem
--      natureza.
--   2. Shiny era impossivel por construcao.
--   3. `coletar_anexo_correio` nao gravava `trait` NEM `nature`. O POKE nascia
--      com `trait = null`, e o cliente cai no fallback do slot 1 da especie
--      (`data/traits.ts#traitDoPoke`) — ou seja, a habilidade OCULTA do Eevee
--      (`anticipation`) era inalcancavel, e `adaptability` (slot 2) tambem. E,
--      pior, a RPC chamava a versao de 10 argumentos de `_calcular_stats`, a que
--      ignora natureza: mesmo se a receita trouxesse `nature`, os stats
--      gravados sairiam sem o multiplicador dela.
--
-- Conferido em producao antes de mexer: 1 concessao (treinador Vinny), Eevee
-- `rarity=comum`, os seis IVs em 23, `trait=null`, `nature=null`.
--
-- =========================================================================
-- AS PROBABILIDADES SAO AS DO JOGO, E O TESTE E QUEM TRANCA ISSO
-- =========================================================================
--
-- Este arquivo repete numeros que vivem no cliente. Isso e divergencia em
-- potencial, e a defesa NAO e boa intencao: `src/data/eeveeSorteado.test.ts` le
-- ESTE SQL com `?raw` e compara com `data/rarity.ts`, `data/natures.ts`,
-- `data/characteristics.ts#IV_MAX`, `data/traits.ts#CHANCE_DE_TRAIT_OCULTA` e
-- `data/generated/traits.generated.ts`. Mudar um peso num lado sem o outro
-- reprova.
--
-- POR QUE NAO LER DO BANCO EM VEZ DE REPETIR: o banco nao tem catalogo de
-- habilidade nenhum. `species` guarda stats, tipos e evolucao; as habilidades e
-- os pesos de raridade so existem em `src/data/`. Trazer as duas tabelas pro
-- Postgres e uma migration de catalogo inteira, com o gerador que a alimenta —
-- e nao e o que esta issue e. A escolha consciente e: repete o minimo (as
-- habilidades de UMA especie e seis pesos), e trava com teste.
--
-- POR QUE `random()` E NAO O RNG SEMEADO DO MOTOR: nao ha mundo aqui. A
-- concessao roda num trigger de `hall_da_fama`, fora de qualquer janela de
-- simulacao, e o resultado e PERSISTIDO na receita do correio — entao nao existe
-- nada pro cliente reconferir nem pro servidor reproduzir. `random()` e a fonte
-- certa exatamente por ser o unico lugar do jogo onde o sorteio nao precisa ser
-- deterministico.
--
-- =========================================================================
-- NIVEL 1, E ISSO MUDOU DEPOIS DA ISSUE
-- =========================================================================
--
-- A concessao de 20260828230000 usava nivel 25 (o nivel do time do Lance).
-- Pedido explicito nesta leva: o Eevee vem NIVEL 1. Efeito colateral bem-vindo:
-- some uma inconsistencia latente da versao anterior — a RPC gravava `exp = 0`
-- junto de `level = 25`, enquanto o cliente (`createPokeInstance`) da a um POKE
-- criado no nivel N a EXP acumulada daquele nivel. No nivel 1 os dois valem 0 e
-- a divergencia deixa de existir.
...
create or replace function public._sortear_eevee_do_lance()
returns jsonb
language plpgsql
volatile
...
begin
  ...
  select s.catch_rate into v_catch_rate from public.species s where s.id = 'eevee';
...
revoke all on function public._sortear_eevee_do_lance() from public;
revoke execute on function public._sortear_eevee_do_lance() from anon;
revoke execute on function public._sortear_eevee_do_lance() from authenticated;
...
drop function if exists public._conceder_eevee_do_lance(uuid);

create or replace function public._conceder_eevee_do_lance(
  p_user_id uuid,
...
begin
  ...
  v_receita := public._sortear_eevee_do_lance();
...
revoke all on function public._conceder_eevee_do_lance(uuid, uuid) from public;
revoke execute on function public._conceder_eevee_do_lance(uuid, uuid) from anon;
revoke execute on function public._conceder_eevee_do_lance(uuid, uuid) from authenticated;
...
begin
  ...
  perform public._conceder_eevee_do_lance(new.user_id, null);
  return new;
end;
$$;
...
begin
  ...
  if v_poke is not null then
    ...
    if v_substitui is not null then
      delete from public.pokemon_instances where id = v_substitui and user_id = v_user_id;
    end if;
