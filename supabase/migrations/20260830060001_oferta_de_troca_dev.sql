-- PH-310 (espelho do schema dev): a OFERTA em cima da mesa, e a reserva do que esta
-- nela.
--
...
alter table dev.troca_sessao
  add column if not exists versao integer not null default 0;

comment on column dev.troca_sessao.versao is
  'PH-310: sobe a cada alteracao da oferta (por trigger). A fatia 3 recusa confirmacao de versao antiga.';

-- ---------------------------------------------------------------------------
-- A oferta
-- ---------------------------------------------------------------------------
create table if not exists dev.troca_oferta (
  id uuid primary key default gen_random_uuid(),
  sessao_id uuid not null references dev.troca_sessao(id) on delete cascade,
  -- De QUEM e o que esta na mesa. Nao da pra derivar da sessao: os dois lados
  -- poem coisa na MESMA mesa.
  dono_id uuid not null references dev.players(user_id) on delete cascade,
  ...
  poke_uid uuid references dev.pokemon_instances(id) on delete cascade,
  item_id text references dev.items(id) on delete restrict,
...
comment on table dev.troca_oferta is
...
create unique index if not exists troca_oferta_item_por_lado
  on dev.troca_oferta (sessao_id, dono_id, item_id)
  where item_id is not null;

create index if not exists troca_oferta_por_sessao
  on dev.troca_oferta (sessao_id);

alter table dev.troca_oferta enable row level security;
...
drop policy if exists "oferta leitura dos participantes" on dev.troca_oferta;
create policy "oferta leitura dos participantes" on dev.troca_oferta
  for select to authenticated
  using (exists (
    select 1 from dev.troca_sessao s
     where s.id = sessao_id
       and (s.anfitriao_id = auth.uid() or s.convidado_id = auth.uid())
  ));

grant select on dev.troca_oferta to authenticated;
grant select, insert, update, delete on dev.troca_oferta to service_role;

-- ---------------------------------------------------------------------------
-- O trigger de versao
-- ---------------------------------------------------------------------------
create or replace function dev.troca_oferta_sobe_versao()
returns trigger
language plpgsql
security definer
...
drop trigger if exists troca_oferta_versao on dev.troca_oferta;
create trigger troca_oferta_versao
  after insert or update or delete on dev.troca_oferta
  for each row execute function dev.troca_oferta_sobe_versao();
...
create or replace function dev._devolver_oferta(p_sessao_id uuid)
returns void
language plpgsql
security definer
set search_path to 'dev'
...
begin
  ...
  update dev.pokemon_instances p
     set location = 'bag', team_slot = null, updated_at = now()
    from dev.troca_oferta o
  ...
  on conflict (user_id, item_id)
    do update set quantity = dev.player_items.quantity + excluded.quantity,
                  updated_at = now();

  delete from dev.troca_oferta where sessao_id = p_sessao_id;
end;
$function$;
...
create or replace function dev._mesa_aberta_minha(p_sessao_id uuid, p_eu uuid)
...
begin
  select * into v_sessao from dev.troca_sessao where id = p_sessao_id for update;
  if not found then
...
create or replace function dev._troca_teto_por_lado()
...
begin
  ...
  insert into dev.troca_oferta (sessao_id, dono_id, tipo, poke_uid)
...
create or replace function dev.tirar_poke_da_mesa(p_sessao_id uuid, p_poke_id uuid)
returns dev.troca_sessao
language plpgsql
security definer
set search_path to 'dev'
...
-- ---------------------------------------------------------------------------
create or replace function dev.por_item_na_mesa(p_sessao_id uuid, p_item_id text, p_quantidade integer)
returns dev.troca_sessao
language plpgsql
security definer
set search_path to 'dev'
...
begin
  ...
  select exists (
    select 1 from dev.troca_oferta
  ...
  insert into dev.troca_oferta (sessao_id, dono_id, tipo, item_id, quantidade)
  values (p_sessao_id, v_eu, 'item', p_item_id, p_quantidade)
  on conflict (sessao_id, dono_id, item_id) where item_id is not null
    do update set quantidade = dev.troca_oferta.quantidade + excluded.quantidade;
...
create or replace function dev.tirar_item_da_mesa(p_sessao_id uuid, p_item_id text, p_quantidade integer)
returns dev.troca_sessao
language plpgsql
security definer
set search_path to 'dev'
...
begin
  ...
  select quantidade into v_na_mesa from dev.troca_oferta
   where sessao_id = p_sessao_id and dono_id = v_eu and item_id = p_item_id
   for update;
  if not found then
...
create or replace function dev.encerrar_troca(p_sessao_id uuid, p_motivo text default 'cancelada')
returns dev.troca_sessao
language plpgsql
security definer
set search_path to 'dev'
...
begin
  ...
  select * into v_sessao from dev.troca_sessao
  ...
  perform dev._devolver_oferta(p_sessao_id);
...
create or replace function dev.expirar_trocas()
returns integer
language plpgsql
security definer
set search_path to 'dev'
...
begin
  ...
  for v_id in
    select id from dev.troca_sessao
     where estado in ('convidada', 'aberta')
       and expira_em <= now()
     for update skip locked
  loop
    perform dev._devolver_oferta(v_id);
    update dev.troca_sessao
...
create or replace function dev.aceitar_troca(p_sessao_id uuid)
returns dev.troca_sessao
language plpgsql
security definer
set search_path to 'dev'
...
grant execute on function dev.por_poke_na_mesa(uuid, uuid) to authenticated;
grant execute on function dev.tirar_poke_da_mesa(uuid, uuid) to authenticated;
grant execute on function dev.por_item_na_mesa(uuid, text, integer) to authenticated;
grant execute on function dev.tirar_item_da_mesa(uuid, text, integer) to authenticated;
...
grant execute on function dev._devolver_oferta(uuid) to service_role;
grant execute on function dev._mesa_aberta_minha(uuid, uuid) to service_role;
grant execute on function dev._troca_teto_por_lado() to authenticated;
