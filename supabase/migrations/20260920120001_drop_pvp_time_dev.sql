-- Gemeo dev de 20260920120000_drop_pvp_time_public.sql.

-- PH-567 (schema dev): fim da transicao da PH-563. Desde a 7.65 nenhum cliente publicado
...
drop function if exists dev.pvp_espelhar_time(uuid);
drop function if exists dev.salvar_time_pvp(uuid[]);
drop function if exists dev.pvp_time_pronto_ranqueado(uuid);
drop table if exists dev.pvp_time;
