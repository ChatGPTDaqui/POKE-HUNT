-- PH-277: sessao de hunt abandonada passa a fechar sozinha.
--
-- O QUE EXISTIA
--
...
create or replace function public.fechar_sessoes_inativas(
...
revoke execute on function public.fechar_sessoes_inativas(interval) from public, anon, authenticated;
grant execute on function public.fechar_sessoes_inativas(interval) to service_role;

-- `unschedule` condicional + `schedule`: idempotentes juntos, e sem depender de
-- o job ja existir. Mesmo padrao de 20260823000000.
--
-- Minuto 29 pra nao competir com `game-sessions-purge` (11), `chat-messages-purge`
-- (17) e `audit-logs-purge` (23) pela mesma janela de conexao do pg_cron.
do $$
begin
  if exists (select 1 from cron.job where jobname = 'sessoes-inativas-fechar') then
    perform cron.unschedule('sessoes-inativas-fechar');
  end if;
end
$$;

select cron.schedule(
  'sessoes-inativas-fechar',
  '29 * * * *',
  $$select public.fechar_sessoes_inativas();$$
);
