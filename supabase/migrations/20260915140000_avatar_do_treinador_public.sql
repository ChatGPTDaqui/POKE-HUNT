...
--
-- Aditiva: coluna `avatar` em public.players, RPC `definir_avatar`, a view
...
alter table public.players drop constraint if exists players_avatar_formato;
alter table public.players add constraint players_avatar_formato
...
create or replace function public.definir_avatar(p_avatar text)
returns jsonb
...
begin
  ...
  update public.players set avatar = v_avatar where user_id = v_user_id;
...
$$;
revoke all on function public.definir_avatar(text) from public;
...
update public.players p
 ...
 where u.id = p.user_id
   and p.user_id in (select user_id from public.pvp_bots)
