...
--
-- Gemeo public: `20260823070000_leilao_de_poke_public.sql`.
--
...
create index if not exists market_listings_leiloes_a_vencer_idx
  on dev.market_listings (expira_em)
...
create or replace function dev.criar_leilao(
  p_poke_id uuid,
  p_currency text,
  p_horas int,
...
begin
  ...
  insert into dev.market_listings (
...
$$;
revoke all on function dev.criar_leilao(uuid, text, int, bigint, bigint) from public;
grant execute on function dev.criar_leilao(uuid, text, int, bigint, bigint) to authenticated;
...
begin
  ...
  select modo, (select count(*) from dev.market_offers o where o.listing_id = l.id and o.status = 'pendente')
...
-- ===========================================================================
create or replace function dev.encerrar_leiloes_vencidos(p_limite int default 200)
returns jsonb
language plpgsql security definer set search_path = dev, public
as $$
declare
  v_leilao record;
  v_vencedora dev.market_offers;
  v_perdedora record;
  v_taxa bigint;
  v_nome text;
  v_encerrados int := 0;
...
begin
  for v_leilao in
    select * from dev.market_listings
  ...
  loop
    ...
    select * into v_vencedora from dev.market_offers
    ...
    if v_vencedora is null then
      ...
      update dev.market_listings set status='expirado' where id = v_leilao.id;
      update dev.pokemon_instances set location='bag', team_slot=null, updated_at=now()
        where id = v_leilao.poke_uid;
      v_sem_lance := v_sem_lance + 1;
      continue;
    end if;

    update dev.market_offers set status='aceita', resolved_at=now() where id = v_vencedora.id;
    update dev.market_listings set status='vendido', sold_at=now(), buyer_id=v_vencedora.buyer_id
      where id = v_leilao.id;
    update dev.pokemon_instances set user_id=v_vencedora.buyer_id, location='bag', team_slot=null, updated_at=now()
      where id = v_leilao.poke_uid;
    ...
    v_taxa := dev.taxa_de_venda(v_vencedora.valor, v_vencedora.currency);
    if v_vencedora.currency = 'gold' then
      update dev.players set gold = gold + (v_vencedora.valor - v_taxa) where user_id = v_leilao.seller_id;
    else
      update dev.players set diamonds = diamonds + (v_vencedora.valor - v_taxa) where user_id = v_leilao.seller_id;
    end if;
    ...
    for v_perdedora in
      ...
      for update
    loop
      update dev.market_offers set status='recusada', resolved_at=now() where id = v_perdedora.id;
      if v_perdedora.currency = 'gold' then
        update dev.players set gold = gold + v_perdedora.valor where user_id = v_perdedora.buyer_id;
      else
        update dev.players set diamonds = diamonds + v_perdedora.valor where user_id = v_perdedora.buyer_id;
      end if;
...
revoke all on function dev.encerrar_leiloes_vencidos(int) from public;
...
select cron.schedule('leiloes-encerrar-dev', '* * * * *', $$select dev.encerrar_leiloes_vencidos();$$);
