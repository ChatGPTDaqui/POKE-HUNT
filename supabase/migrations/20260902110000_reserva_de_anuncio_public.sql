...
comment on column public.market_listings.reservado_para is
...
create index if not exists market_listings_reservados_idx
  on public.market_listings (reservado_para)
...
create view public.mercado_anuncios_ativos as
select
  ...
  (select count(*) from public.market_offers o
     where o.listing_id = l.id and o.status = 'pendente')::int as ofertas,
  (select max(valor) from public.market_offers o
...
left join public.treinadores_publico r on r.user_id = l.reservado_para
...
alter view public.mercado_anuncios_ativos set (security_invoker = false);
...
-- estiver valendo.
create or replace function public.reservar_anuncio(
  p_anuncio_id uuid,
...
begin
  ...
  -- nunca vai acontecer.
  select count(*) into v_ofertas from public.market_offers
  ...
  if p_para_id is null then
    update public.market_listings set reservado_para = null where id = p_anuncio_id;
  ...
  select user_id, trainer_name into v_destino from public.players where user_id = p_para_id;
  ...
  if v_mudou then
    ...
    from public.players p where p.user_id = v_user_id;
  end if;
...
revoke all on function public.reservar_anuncio(uuid, uuid, int) from public;
revoke execute on function public.reservar_anuncio(uuid, uuid, int) from anon;
grant execute on function public.reservar_anuncio(uuid, uuid, int) to authenticated;
