-- PH-539: bots do PvP ranqueado.
--
-- Sem base de jogadores a fila ranqueada nunca casa ninguem. Dez treinadores
-- iconicos viram contas reais (auth.users + players, semeadas por
-- scripts/pvp/seed-bots.mjs) marcadas em `pvp_bots`. `tentar_parear_ranqueado`
-- ganha um fallback: sem humano na janela de MMR e com 15s de espera, casa com
-- um bot aleatorio no modo `ranqueado_bot`.
--
-- `ranqueado_bot` NAO mexe em MMR/PDL/divisao nem consome `partidas_hoje`:
-- `aplicar_resultado_pvp` e a edge function ja tratam qualquer modo diferente
-- de 'ranqueado' como partida sem stakes. O historico grava normal.
--
-- Aditiva: so amplia o CHECK de `modo`, cria `pvp_bots`, expoe `eh_bot` em
-- `treinadores_publico` e tira bot do `ranking_pokemon` (time nivel 80 com
-- IV 31 e raridade legendary tomaria o top 50 inteiro).
...
drop policy if exists "pvp_bots leitura publica" on public.pvp_bots;
create policy "pvp_bots leitura publica" on public.pvp_bots
  for select to authenticated
  using (true);
grant select on public.pvp_bots to authenticated;
grant select, insert, update, delete on public.pvp_bots to service_role;

alter table public.pvp_sessao drop constraint if exists pvp_sessao_modo_check;
alter table public.pvp_sessao
  add constraint pvp_sessao_modo_check check (modo in ('amistoso', 'ranqueado', 'ranqueado_bot'));

alter table public.pvp_historico drop constraint if exists pvp_historico_modo_check;
...
create or replace view public.treinadores_publico as
  select p.user_id, p.trainer_name, p.trainer_level, p.trainer_exp,
         exists (select 1 from public.pvp_bots b where b.user_id = p.user_id) as eh_bot
  from public.players p;

create or replace view public.ranking_pokemon as
with elegivel as (
  ...
  join public.treinadores_publico t on t.user_id = pi.user_id and not t.eh_bot
),
top as (
...
begin
  ...
  update public.pvp_sessao
   ...
   where estado in ('convidada', 'aberta')
     and expira_em <= now()
     and convidado_id in (select user_id from public.pvp_bots);

  select b.user_id into v_bot
    from public.pvp_bots b
   where exists (
           select 1 from public.pvp_time t
            where t.user_id = b.user_id
              and coalesce(array_length(t.pokemon_ids, 1), 0) = 6
         )
     and not exists (
           select 1 from public.pvp_sessao s
  ...
  begin
    delete from public.pvp_fila where user_id = v_eu;
    ...
    values (
      ...
      public.pvp_snapshot_time(v_eu), public.pvp_snapshot_time(v_bot),
      null, null
    )
    returning * into v_s;
