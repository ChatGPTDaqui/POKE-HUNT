-- PH-334 (schema dev): `sala_protetor` guardava uma linha por sessao fechada e nunca limpava.
--
...
create or replace function dev.purgar_sala_protetor(
...
begin
  ...
  delete from dev.sala_protetor sp
   where not exists (
     select 1
       from dev.game_sessions gs
...
revoke execute on function dev.purgar_sala_protetor(interval) from public, anon, authenticated;
grant execute on function dev.purgar_sala_protetor(interval) to service_role;

-- `unschedule` condicional + `schedule`: idempotentes juntos, e sem depender de
-- o job ja existir. Mesmo padrao de 20260830010000.
--
-- Minuto 04 pra nao competir com os purges de hora em hora que ja existem —
-- `game-sessions-purge-dev` (41), `chat-messages-purge-dev` (47),
-- `audit-logs-purge-dev` (53) e `sessoes-inativas-fechar-dev` (59) — nem com os
-- `*/5` (`leiloes-encerrar` em 0,5,10... e `trocas-expirar` em 2,7,12...).
do $$
begin
  if exists (select 1 from cron.job where jobname = 'sala-protetor-purge-dev') then
    perform cron.unschedule('sala-protetor-purge-dev');
  end if;
end
$$;

select cron.schedule(
  'sala-protetor-purge-dev',
  '04 * * * *',
  $$select dev.purgar_sala_protetor();$$
);

-- BACKFILL das 7 orfas de hoje. E a PROPRIA funcao, entao nao ha uma segunda
