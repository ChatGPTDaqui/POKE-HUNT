-- Gemeo dev de 20260920100000_pvp_ranqueado_assincrono_public.sql.

-- PH-565 (schema dev): ranqueado ASSINCRONO. Procurar partida vira "atacar a defesa salva
...
drop index if exists dev.pvp_sessao_convidado_viva;
...
create or replace function dev.pvp_preset_pronto_ranqueado(p_user uuid, p_tipo text)
returns boolean
language sql
stable
set search_path to 'dev'
as $function$
  select jsonb_array_length(dev.pvp_snapshot_preset(p_user, p_tipo)) >= 1;
$function$;
revoke execute on function dev.pvp_preset_pronto_ranqueado(uuid, text) from public;

-- 3. Atacar.
create or replace function dev.atacar_ranqueado()
...
begin
  ...
  if not dev.pvp_preset_pronto_ranqueado(v_eu, 'ataque') then
  ...
  v_ataque := dev.pvp_snapshot_preset(v_eu, 'ataque');
    ...
    for v_cand in
      select r.user_id
       ...
       where r.user_id <> v_eu
         and not exists (select 1 from dev.pvp_bots b where b.user_id = r.user_id)
         and abs(r.mmr - v_rank.mmr) <= v_janela
         and exists (
               select 1 from dev.pvp_preset d
                where d.user_id = r.user_id and d.tipo = 'defesa' and d.ativo
                  and jsonb_array_length(d.slots) > 0
             )
         and r.user_id not in (
               select s.convidado_id from dev.pvp_sessao s
       ...
       limit 10
    loop
      v_defesa := dev.pvp_snapshot_preset(v_cand.user_id, 'defesa');
  ...
  if v_alvo is not null then
    ...
    v_defesa := dev.pvp_snapshot_preset(v_alvo, 'defesa');
  ...
  update dev.pvp_rank set partidas_hoje = partidas_hoje + 1 where user_id = v_eu;

  return v_s;
end
$function$;
grant execute on function dev.atacar_ranqueado() to authenticated;

-- 4. A fila morre.
drop function if exists dev.tentar_parear_ranqueado();
drop function if exists dev.entrar_fila_ranqueada();
drop function if exists dev.sair_da_fila_ranqueada();
drop table if exists dev.pvp_fila;
