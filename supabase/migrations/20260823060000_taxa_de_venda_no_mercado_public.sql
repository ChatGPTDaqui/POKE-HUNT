...
--
-- Gemeo dev: `20260823060001_taxa_de_venda_no_mercado_dev.sql`.
--
-- ===========================================================================
...
comment on column public.market_trades.taxa is
...
-- ===========================================================================
create or replace function public.taxa_do_mercado()
returns jsonb
...
comment on function public.taxa_do_mercado() is
...
create or replace function public.taxa_de_venda(p_valor bigint, p_currency text)
returns bigint
language plpgsql immutable
set search_path = public
as $$
declare
  v_regra jsonb := public.taxa_do_mercado();
begin
...
grant execute on function public.taxa_do_mercado() to authenticated;
...
begin
  ...
  loop
    ...
    v_taxa := public.taxa_de_venda(v_valor, 'gold');
    ...
    if p_side = 'compra' then
      ...
      update public.players set gold = gold + (v_valor - v_taxa) where user_id = v_candidata.user_id;
    else
      ...
      update public.players set gold = gold + (v_valor - v_taxa) where user_id = v_user_id;
    ...
    insert into public.market_trades (kind, item_id, quantity, unit_price, currency, buyer_id, seller_id, taxa)
...
begin
  ...
  v_taxa := public.taxa_de_venda(v_anuncio.price::bigint, v_anuncio.currency);
  if v_anuncio.currency = 'gold' then
    update public.players set gold = gold + (v_anuncio.price - v_taxa) where user_id = v_anuncio.seller_id;
  else
    ...
    update public.players set diamonds = diamonds + (v_anuncio.price - v_taxa) where user_id = v_anuncio.seller_id;
  end if;
...
begin
  ...
  v_taxa := public.taxa_de_venda(v_oferta.valor::bigint, v_oferta.currency);
  if v_oferta.currency = 'gold' then
    update public.players set gold = gold + (v_oferta.valor - v_taxa) where user_id = v_user_id;
  else
    update public.players set diamonds = diamonds + (v_oferta.valor - v_taxa) where user_id = v_user_id;
  end if;
