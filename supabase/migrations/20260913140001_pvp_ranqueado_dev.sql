-- Gemeo dev de 20260913140000_pvp_ranqueado_public.sql.
create table if not exists dev.pvp_time (
  user_id uuid primary key references dev.players(user_id) on delete cascade,
  pokemon_ids uuid[] not null default '{}',
  atualizado_em timestamptz not null default now()
);

alter table dev.pvp_time enable row level security;
drop policy if exists "pvp_time leitura do dono" on dev.pvp_time;
create policy "pvp_time leitura do dono" on dev.pvp_time
  for select to authenticated
  using (user_id = auth.uid());
grant select on dev.pvp_time to authenticated;
grant select, insert, update on dev.pvp_time to service_role;

create table if not exists dev.pvp_rank (
  user_id uuid primary key references dev.players(user_id) on delete cascade,
...
alter table dev.pvp_rank enable row level security;
drop policy if exists "pvp_rank leitura do dono" on dev.pvp_rank;
create policy "pvp_rank leitura do dono" on dev.pvp_rank
  for select to authenticated
  using (user_id = auth.uid());
grant select on dev.pvp_rank to authenticated;
grant select, insert, update on dev.pvp_rank to service_role;

create table if not exists dev.pvp_fila (
  user_id uuid primary key references dev.players(user_id) on delete cascade,
  mmr integer not null,
  entrou_em timestamptz not null default now()
);

alter table dev.pvp_fila enable row level security;
drop policy if exists "pvp_fila leitura do dono" on dev.pvp_fila;
create policy "pvp_fila leitura do dono" on dev.pvp_fila
  for select to authenticated
  using (user_id = auth.uid());
grant select on dev.pvp_fila to authenticated;
grant select, insert, update, delete on dev.pvp_fila to service_role;
...
create or replace function dev.pvp_proximo_reset(p_ref timestamptz default now())
returns timestamptz
language sql
stable
as $function$
...
create or replace function dev.pvp_garantir_rank(p_user uuid)
...
declare
  v_r dev.pvp_rank;
begin
  insert into dev.pvp_rank (user_id, reset_em)
  values (p_user, dev.pvp_proximo_reset())
  on conflict (user_id) do nothing;

  select * into v_r from dev.pvp_rank where user_id = p_user for update;

  if v_r.reset_em <= now() then
    update dev.pvp_rank
       set partidas_hoje = 0, reset_em = dev.pvp_proximo_reset()
...
create or replace function dev.meu_rank_pvp()
returns dev.pvp_rank
language plpgsql
security definer
set search_path to 'dev'
...
begin
  ...
  return dev.pvp_garantir_rank(v_eu);
end
$function$;

create or replace function dev.salvar_time_pvp(p_pokemon_ids uuid[])
returns dev.pvp_time
language plpgsql
security definer
set search_path to 'dev'
as $function$
...
begin
  ...
  select array_agg(distinct id) into v_ids
    ...
    where exists (select 1 from dev.pokemon_instances pi where pi.id = id and pi.user_id = v_eu);
  ...
  insert into dev.pvp_time (user_id, pokemon_ids, atualizado_em)
...
as $function$
  select coalesce(array_length(t.pokemon_ids, 1), 0) = 6
     and not exists (
       ...
       join dev.pokemon_instances pi on pi.id = pid
...
create or replace function dev.sair_da_fila_ranqueada()
returns void
language sql
security definer
set search_path to 'dev'
as $function$
  delete from dev.pvp_fila where user_id = auth.uid();
$function$;
...
create or replace function dev.aplicar_resultado_pvp(
  p_sessao_id uuid,
  p_vencedor_id uuid,
  p_eventos jsonb,
...
grant execute on function dev.meu_rank_pvp() to authenticated;
grant execute on function dev.salvar_time_pvp(uuid[]) to authenticated;
grant execute on function dev.entrar_fila_ranqueada() to authenticated;
grant execute on function dev.sair_da_fila_ranqueada() to authenticated;
grant execute on function dev.tentar_parear_ranqueado() to authenticated;
revoke execute on function dev.pvp_garantir_rank(uuid) from public;
revoke execute on function dev.pvp_snapshot_time(uuid) from public;
revoke execute on function dev.pvp_time_pronto_ranqueado(uuid) from public;
revoke execute on function dev.aplicar_resultado_pvp(uuid, uuid, jsonb, integer, integer, text, integer, integer, text, integer, integer) from public, authenticated;
grant execute on function dev.aplicar_resultado_pvp(uuid, uuid, jsonb, integer, integer, text, integer, integer, text, integer, integer) to service_role;

do $$
begin
  if not exists (
    select 1 from pg_publication_tables
     where pubname = 'supabase_realtime' and schemaname = 'dev' and tablename = 'pvp_rank'
  ) then
    alter publication supabase_realtime add table dev.pvp_rank;
  end if;
end $$;
