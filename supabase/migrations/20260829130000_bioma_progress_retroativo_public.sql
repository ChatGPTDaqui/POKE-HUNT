-- PH-284 (dado): credita retroativamente `players.bioma_progress` a quem ja
-- fechou ciclo de 10 salas enquanto a RPC `gravar_progresso` descartava a
-- coluna (ver 20260829120000_gravar_progresso_grava_bioma_progress_public.sql).
--
...
declare
  ...
  v_novo jsonb;
begin
  for v_jogador in select user_id, bioma_progress from public.players loop
    ...
    if v_novo is distinct from v_jogador.bioma_progress then
      update public.players set bioma_progress = v_novo where user_id = v_jogador.user_id;
    end if;
  end loop;
end $$;
