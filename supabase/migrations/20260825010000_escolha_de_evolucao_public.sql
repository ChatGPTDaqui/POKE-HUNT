-- PH-139 -- especie com MAIS DE UM destino de evolucao, e o jogador escolhendo
-- qual.
--
-- ---------------------------------------------------------------------------
...
create table if not exists public.species_evolution_options (
  species_id text not null references public.species(id) on delete cascade,
  evolves_to text not null references public.species(id) on delete cascade,
...
comment on table public.species_evolution_options is
  'PH-139: destinos de evolucao de uma especie que tem MAIS DE UM. Especie de ramo unico continua so em species.evolves_to.';

alter table public.species_evolution_options enable row level security;

-- Catalogo e dado publico de leitura, igual `species` — a tela precisa saber
-- quais sao as opcoes antes de chamar a RPC.
drop policy if exists "opcoes de evolucao sao publicas" on public.species_evolution_options;
create policy "opcoes de evolucao sao publicas" on public.species_evolution_options
  for select to authenticated using (true);
grant select on public.species_evolution_options to authenticated;
...
-- causa disso.
insert into public.species_evolution_options (species_id, evolves_to, evolves_at_level, is_special_evolution, ordem)
...
revoke all on function public.evoluir_poke(uuid, text) from public;
grant execute on function public.evoluir_poke(uuid, text) to authenticated;
...
as $$
begin
  return public.evoluir_poke(p_poke_id, null);
end;
$$;
