-- Auto-venda no schema `dev` — o par de 20260818090000_auto_sell_config_public.
--
-- `dev` e o clone de teste (ver docs/): as RPCs existem nos dois schemas e
-- precisam andar juntas, senao testar em `dev` valida um comportamento que
-- producao nao tem. Este corpo foi tirado da definicao `dev` que estava
-- publicada (comentarios dela inclusive) mais o bloco `sellConfig` — nao da
-- versao `public`, que tem comentarios proprios.
