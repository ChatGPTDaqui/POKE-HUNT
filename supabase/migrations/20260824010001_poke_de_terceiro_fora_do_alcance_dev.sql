...
-- O FURO
-- ------
-- `20260811203333_rls_leitura_publica_dev.sql` criou:
--
...
-- `20260807030000_cliente_perde_a_escrita.sql` (clonada pra `dev`)) passa a ser a unica leitura de
...
drop policy if exists "pokemon leitura publica" on dev.pokemon_instances;
...
drop policy if exists "jogador le os proprios pokemon" on dev.pokemon_instances;
create policy "jogador le os proprios pokemon" on dev.pokemon_instances
...
drop view if exists dev.ranking_pokemon;

create view dev.ranking_pokemon as
with elegivel as (
...
alter view dev.ranking_pokemon set (security_invoker = false);
