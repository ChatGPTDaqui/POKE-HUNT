...
-- (20260814120100).
--
-- POR QUE DUAS VEZES: `dev` e um clone completo de `public`, com os SEUS
-- PROPRIOS tipos enum (confirmado no banco: element_type existe nos dois
-- schemas). Mexer so num deixaria o sandbox de dev divergindo do que a
-- producao roda — que e exatamente o problema que o clone existe pra evitar.
--
-- FAIRY: a base de dados do jogo passou a ser Pokemon Ultra Sun (Gen VII), que
-- tem 18 tipos. Sem este valor, `stone_fairy` nao pode existir na tabela
-- `items` — e sem `stone_fairy` toda conta que matasse um POKE de tipo Fada
-- travava (ver a nota de bug em 20260814120100).
alter type public.element_type add value if not exists 'FAIRY';
alter type dev.element_type    add value if not exists 'FAIRY';
...
alter type public.item_kind add value if not exists 'status_heal';
alter type dev.item_kind    add value if not exists 'status_heal';
