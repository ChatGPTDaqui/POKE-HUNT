-- PH-314 (espelho do schema dev): a mesa de troca entra no Realtime.
--
...
begin
  if not exists (
    select 1 from pg_publication_tables
     where pubname = 'supabase_realtime' and schemaname = 'dev' and tablename = 'troca_sessao'
  ) then
    alter publication supabase_realtime add table dev.troca_sessao;
  end if;
end;
$$;
