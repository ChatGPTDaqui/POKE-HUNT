-- Gemeo dev de 20260919120000_pvp_preset_public.sql.

-- PH-563 (schema dev): presets de time de PvP (3 de ataque + 3 de defesa) com golpes por slot.
--
...
create table if not exists dev.pvp_preset (
...
create unique index if not exists pvp_preset_um_ativo_por_tipo
  on dev.pvp_preset (user_id, tipo) where ativo;

alter table dev.pvp_preset enable row level security;
drop policy if exists "pvp_preset leitura do dono" on dev.pvp_preset;
create policy "pvp_preset leitura do dono" on dev.pvp_preset
  for select to authenticated
  using (user_id = auth.uid());
grant select on dev.pvp_preset to authenticated;
grant select, insert, update, delete on dev.pvp_preset to service_role;
...
insert into dev.pvp_preset (user_id, tipo, posicao, nome, slots, ativo)
...
create or replace function dev.pvp_espelhar_time(p_user uuid)
returns void
language plpgsql
security definer
...
$function$;
revoke execute on function dev.pvp_espelhar_time(uuid) from public;

-- Garante as 6 linhas do jogador (3 ataque + 3 defesa) e 1 ativo por tipo.
create or replace function dev.pvp_garantir_presets(p_user uuid)
returns void
language plpgsql
security definer
set search_path to 'dev'
as $function$
begin
  insert into dev.pvp_preset (user_id, tipo, posicao)
  select p_user, tipo, posicao
    from (values ('ataque'), ('defesa')) as t(tipo)
   cross join generate_series(1, 3) as posicao
  on conflict (user_id, tipo, posicao) do nothing;

  update dev.pvp_preset pr
     set ativo = true
   where pr.user_id = p_user and pr.posicao = 1
     and not exists (select 1 from dev.pvp_preset o
                      where o.user_id = p_user and o.tipo = pr.tipo and o.ativo);
end
$function$;
revoke execute on function dev.pvp_garantir_presets(uuid) from public;

create or replace function dev.meus_presets_pvp()
returns setof dev.pvp_preset
language plpgsql
security definer
set search_path to 'dev'
as $function$
...
begin
  ...
  perform dev.pvp_garantir_presets(v_eu);
  return query select * from dev.pvp_preset where user_id = v_eu order by tipo, posicao;
end
$function$;
grant execute on function dev.meus_presets_pvp() to authenticated;
...
create or replace function dev.salvar_preset_pvp(
  p_tipo text,
  p_posicao integer,
  p_nome text,
  p_slots jsonb
)
returns dev.pvp_preset
language plpgsql
security definer
set search_path to 'dev'
as $function$
...
begin
  ...
  if v_pr.tipo = 'ataque' and v_pr.ativo then
    perform dev.pvp_espelhar_time(v_eu);
  end if;
  return v_pr;
end
$function$;
grant execute on function dev.salvar_preset_pvp(text, integer, text, jsonb) to authenticated;

create or replace function dev.ativar_preset_pvp(p_tipo text, p_posicao integer)
returns dev.pvp_preset
language plpgsql
security definer
set search_path to 'dev'
...
begin
  ...
  update dev.pvp_preset set ativo = false
   where user_id = v_eu and tipo = p_tipo and ativo and posicao <> p_posicao;
  update dev.pvp_preset set ativo = true, atualizado_em = now()
   where user_id = v_eu and tipo = p_tipo and posicao = p_posicao
  returning * into v_pr;
...
$function$;
grant execute on function dev.ativar_preset_pvp(text, integer) to authenticated;
...
begin
  ...
  perform dev.pvp_garantir_presets(v_eu);
  select posicao into v_posicao from dev.pvp_preset
  ...
  perform dev.pvp_espelhar_time(v_eu);
  select * into v_t from dev.pvp_time where user_id = v_eu;
  return v_t;
end
$function$;
...
create or replace function dev.pvp_snapshot_preset(p_user uuid, p_tipo text)
returns jsonb
language sql
stable
set search_path to 'dev'
...
as $function$
declare
  v jsonb := dev.pvp_snapshot_preset(p_user, 'ataque');
begin
