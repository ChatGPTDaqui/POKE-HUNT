-- PH-119 -- espelho do 20260825030000_perfil_publico_public no schema `dev`.
...
create or replace function dev.perfil_publico(p_user_id uuid)
returns jsonb
...
begin
  ...
  select p.trainer_name, p.trainer_level, p.trainer_exp, p.created_at
    ...
    from dev.players p where p.user_id = p_user_id;
  ...
  select coalesce(sum(simulated_seconds), 0) into v_segundos
    from dev.game_sessions where user_id = p_user_id;
  select segundos into v_arquivados
    from dev.tempo_jogado_arquivado where user_id = p_user_id;
  v_segundos := v_segundos + coalesce(v_arquivados, 0);

  select conquistado_em into v_hall from dev.hall_da_fama where user_id = p_user_id limit 1;

  select count(*) into v_capturas from dev.pokemon_instances where user_id = p_user_id;
  select count(*) into v_anuncios
    from dev.market_listings where seller_id = p_user_id and status = 'ativo';
...
comment on function dev.perfil_publico(uuid) is
  'PH-119: o que um jogador pode ver de OUTRO. Nunca carteira, mochila, e-mail ou time.';

revoke all on function dev.perfil_publico(uuid) from public;
revoke execute on function dev.perfil_publico(uuid) from anon;
grant execute on function dev.perfil_publico(uuid) to authenticated;
