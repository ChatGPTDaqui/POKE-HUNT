...
create or replace function public._mult_natureza(p_nature text, p_stat text)
returns numeric
language plpgsql
immutable
...
revoke all on function public._mult_natureza(text, text) from public;
revoke all on function public._mult_natureza(text, text) from anon;
revoke all on function public._mult_natureza(text, text) from authenticated;
...
create or replace function public._calcular_stats(
...
begin
  return query select
    greatest(1, round(public._calcular_stat(p_species.base_hp, p_level, p_iv_hp, true) * public._mult_natureza(p_nature, 'hp') * v_shiny_mult * v_rarity_mult))::int,
    greatest(1, round(public._calcular_stat(p_species.base_atk_fis, p_level, p_iv_atk_fis, false) * public._mult_natureza(p_nature, 'atk_fis') * v_shiny_mult * v_rarity_mult))::int,
    greatest(1, round(public._calcular_stat(p_species.base_atk_esp, p_level, p_iv_atk_esp, false) * public._mult_natureza(p_nature, 'atk_esp') * v_shiny_mult * v_rarity_mult))::int,
    greatest(1, round(public._calcular_stat(p_species.base_def, p_level, p_iv_def, false) * public._mult_natureza(p_nature, 'def') * v_shiny_mult * v_rarity_mult))::int,
    greatest(1, round(public._calcular_stat(p_species.base_def_esp, p_level, p_iv_def_esp, false) * public._mult_natureza(p_nature, 'def_esp') * v_shiny_mult * v_rarity_mult))::int,
    greatest(1, round(public._calcular_stat(p_species.base_speed, p_level, p_iv_speed, false) * public._mult_natureza(p_nature, 'speed') * v_shiny_mult * v_rarity_mult))::int;
end;
$fn$;

revoke all on function public._calcular_stats(public.species, int, int, int, int, int, int, int, text, boolean, text) from public;
revoke all on function public._calcular_stats(public.species, int, int, int, int, int, int, int, text, boolean, text) from anon;
revoke all on function public._calcular_stats(public.species, int, int, int, int, int, int, int, text, boolean, text) from authenticated;
...
set search_path = public
as $fn$
  select * from public._calcular_stats(
