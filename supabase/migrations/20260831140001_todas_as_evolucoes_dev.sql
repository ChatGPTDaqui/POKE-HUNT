-- PH-145 -- espelho do 20260831140000_todas_as_evolucoes_public no schema `dev`.
...
alter table dev.species_evolution_options
  add column if not exists stone_type public.element_type;

comment on column dev.species_evolution_options.stone_type is
...
-- essas 162 na tabela de opcoes seria duplicar o catalogo sem ganho.
update dev.species s set
...
from (values
  ...
  ('treecko', 'grovyle', 16, false),
  ('grovyle', 'sceptile', 36, false),
  ('torchic', 'combusken', 16, false),
  ('combusken', 'blaziken', 36, false),
  ('mudkip', 'marshtomp', 16, false),
  ('marshtomp', 'swampert', 36, false),
  ('poochyena', 'mightyena', 18, false),
  ('zigzagoon', 'linoone', 20, false),
  ('silcoon', 'beautifly', 10, false),
  ('cascoon', 'dustox', 10, false),
  ('lotad', 'lombre', 14, false),
  ('lombre', 'ludicolo', 80, true),
  ('seedot', 'nuzleaf', 14, false),
  ('nuzleaf', 'shiftry', 80, true),
  ('taillow', 'swellow', 22, false),
  ('wingull', 'pelipper', 25, false),
  ('ralts', 'kirlia', 20, false),
  ('kirlia', 'gardevoir', 30, false),
  ('surskit', 'masquerain', 22, false),
  ('shroomish', 'breloom', 23, false),
  ('slakoth', 'vigoroth', 18, false),
  ('vigoroth', 'slaking', 36, false),
  ('whismur', 'loudred', 20, false),
  ('loudred', 'exploud', 40, false),
  ('makuhita', 'hariyama', 24, false),
  ('azurill', 'marill', 80, true),
  ('skitty', 'delcatty', 80, true),
  ('aron', 'lairon', 32, false),
  ('lairon', 'aggron', 42, false),
  ('meditite', 'medicham', 37, false),
  ('electrike', 'manectric', 26, false),
  ('gulpin', 'swalot', 26, false),
  ('carvanha', 'sharpedo', 30, false),
  ('wailmer', 'wailord', 40, false),
  ('numel', 'camerupt', 33, false),
  ('spoink', 'grumpig', 32, false),
  ('trapinch', 'vibrava', 35, false),
  ('vibrava', 'flygon', 45, false),
  ('cacnea', 'cacturne', 32, false),
  ('swablu', 'altaria', 35, false),
  ('barboach', 'whiscash', 30, false),
  ('corphish', 'crawdaunt', 30, false),
  ('baltoy', 'claydol', 36, false),
  ('lileep', 'cradily', 40, false),
  ('anorith', 'armaldo', 40, false),
  ('feebas', 'milotic', 80, true),
  ('shuppet', 'banette', 37, false),
  ('duskull', 'dusclops', 37, false),
  ('wynaut', 'wobbuffet', 15, false),
  ('snorunt', 'glalie', 42, false),
  ('spheal', 'sealeo', 32, false),
  ('sealeo', 'walrein', 44, false),
  ('bagon', 'shelgon', 30, false),
  ('shelgon', 'salamence', 50, false),
  ('beldum', 'metang', 20, false),
  ('metang', 'metagross', 45, false)
) as v(species_id, evolves_to, evolves_at_level, is_special_evolution)
where s.id = v.species_id
  and exists (select 1 from dev.species d where d.id = v.evolves_to)
...
-- ---------------------------------------------------------------------------
-- gloom (2), poliwhirl (2), slowpoke (2), eevee (5), tyrogue (3), wurmple (2), nincada (2), clamperl (2).
--
...
insert into dev.species_evolution_options
...
from (values
  ...
  ('wurmple', 'silcoon', 7, false, 0, null::public.element_type),
  ('wurmple', 'cascoon', 7, false, 1, null::public.element_type),
  ('nincada', 'ninjask', 20, false, 0, null::public.element_type),
  ('nincada', 'shedinja', 80, true, 1, 'BUG'::public.element_type),
  ('clamperl', 'huntail', 80, true, 0, 'WATER'::public.element_type),
  ('clamperl', 'gorebyss', 80, true, 1, 'WATER'::public.element_type)
...
from (
  ...
  from dev.species_evolution_options order by species_id, ordem
) o
...
returns jsonb
language plpgsql security definer set search_path = dev
as $$
declare
