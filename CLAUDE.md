...
- **"Boss" no projeto são TRÊS sistemas distintos, não dois — não confundir nenhum dos três.** (1) **Sistema de sala das hunts** (mini/ultimate por sala, PH-223→230): NUNCA usa a palavra "boss" (nem "chefe") em nome de identificador — decisão explícita do usuário (28/08), pra zerar colisão com o (2). Nomenclatura: **Guardian** (mini, sala 1-9) e **Lord** (ultimate, sala 10). PH-236 é o rename completo (engine, banco — tabela nova dedicada, não coluna `boss_*` em `game_sessions` — authority, testes, UI); até ele fechar, o código ainda tem `bossDaSala`/`TipoDeBoss`/`boss_*` como legado histórico — não copiar esse padrão de nome em código novo, e qualquer PR que toque nesta área deve seguir Guardian/Lord, não "boss". (2) **Boss global** — feature separada do Marcos, totalmente fora deste repo/escopo, livre pra usar "boss" no nome dela. (3) **"Hunts BOSS" / Modo Pesadelo** (`data/legendaries.ts#LEGENDARY_SPECIES_IDS`, documentado em `docs/06-mundo-hunts-e-spawn.md`): 11 hunts dedicadas, uma por lendário, sistema PRÉ-EXISTENTE e mais antigo que (1) — mantém o nome "BOSS" como está, não é afetado pelo PH-236 nem pela regra do (1), só por acaso compartilha helper visual em `src/render/sprites.ts`.
