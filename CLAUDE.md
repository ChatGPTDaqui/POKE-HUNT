...
npm run usum:gerar      # gera catálogo atual USUM (Gen VII)
```
...
A fonte vigente do processo é [docs/PROCESSO.md](docs/PROCESSO.md): Jira, verificação proporcional, banco, PR e promoção. Leia antes de executar tarefas.

`CLAUDE.local.md` contém caminhos pessoais opcionais. `docs/README.md` indexa a documentação técnica; `HISTORICO.md` e Obsidian preservam contexto histórico, sob demanda.
