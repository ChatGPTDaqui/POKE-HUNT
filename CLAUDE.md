...
npm run build:verificar # tsc -b + vite build, sem a copia — o de CONFERIR
npm test                # vitest run
...
- **Promover não acaba no merge — quem promove tem QUATRO obrigações depois dele, e elas não são opcionais.**
  ...
  2. **Abrir o jogo em produção e confirmar que ele CARREGA** — não que a tela sobe. `node scripts/harness/fumaca-de-producao.mjs` **e** `node scripts/harness/abrir-hunt-em-producao.mjs`: a primeira sozinha já deu TUDO OK com o jogo trancado. Abrir a página no navegador não substitui nenhuma das duas.
