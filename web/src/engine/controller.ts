// Port da parte de logica pura de js/main.js: construcao de mundo
// (buildHospitalWorld/buildMapWorld/spawnEnemyAt/sequencia da Lance),
// stepWorld (a funcao-motor unica) e as acoes de `controller` que main.js
// original expunha pra UI (chooseStarter/enterMap/returnToHospital/
// healTeam/setActiveTeamIndex/removeFromTeam/useItem/evolvePoke/resetGame/
// resetPerfStats). NAO inclui a parte DOM/canvas pura do main.js original
// (resize de canvas, hit-test de clique na enfermeira, zoom, fade de
// transicao de tela, setInterval/beforeunload de autosave) — isso e Fase 5
// (GameCanvas/App), que chama as funcoes daqui de dentro de effects.
//
// Decisao de arquitetura importante (nao e so port mecanico, ver relatorio
// final): no jogo vanilla, `player.poke` e `gameState.activePoke` eram O
// MESMO objeto JS (referencia compartilhada) — dano/cura/EXP durante o
// combate atualizavam os dois automaticamente de graca. Com o motor
// rodando em duas stores Zustand separadas (worldStore efemera/60Hz,
// gameStateStore persistente), essa referencia compartilhada nao existe
// mais. Resolvido assim: **worldStore.player.poke e a copia "ao vivo"
// autoritativa durante uma hunt** (HP/EXP mudam nela a cada tick de
// combate); **gameStateStore.team e sincronizada explicitamente** nos
// pontos que ja importavam pra persistencia no original — a cada kill
// (grantExp ja teria que escrever de volta de qualquer jeito, pra
// persistir nivel/stats/habilidades), a cada faint, e via uma sincronia
// periodica de baixa frequencia que a Fase 5 liga (nao every-tick, pra nao
// martelar o localStorage a 60Hz via zustand/persist). HUD (Fase 6) deve
// ler HP ao vivo de worldStore, nao de gameStateStore, exatamente por isso.
...
import { grantExp, expRewardForEnemy, evolvePokeInstance, grantTrainerExp, applyDeathExpPenalty } from './systems/progressionSystem'
import { awardKillLoot } from './systems/economySystem'
import { recordKill, resetStats } from './systems/statsTracker'
import { recordPokedexKill } from './systems/pokedexSystem'
import type { KillResult } from './systems/offlineSimSystem'

import { useGameStateStore, type GameStateStore } from '@/stores/gameStateStore'
import { useWorldStore, emptyWorldState } from '@/stores/worldStore'
...
const STARTER_RARITY = 'comum' as const
...
function shinyPrefix(isShiny?: boolean): string {
  return isShiny ? '✨ ' : ''
}
  ...
  if (playerJustFainted && world.player) {
    ...
    if (world.mapDef.autoSwitchTeamOnFaint) {
      ...
      if (nextIndex !== -1) {
        ...
        if (!silent) {
          useToastStore.getState().pushToast(`${shinyPrefix(nextPoke.isShiny)}${SPECIES[nextPoke.speciesId].name} entrou em campo!`, 'success', 'combat')
        }
      }
    }
  }
